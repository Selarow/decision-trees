...
class MultivariateDecisionTree():
    ...
    def split(self, dataset, t1, feature1, t2, feature2, unary):
        if unary:
            left = dataset[(dataset[feature1] <= t1)]
            right = dataset[(dataset[feature1] > t1)]
                ...
                for t1 in thresholds1:
                    for t2 in thresholds2:
                        ...
                        if feature1 != feature2:
                            unary = False
                            left, right = self.split(dataset, t1, feature1, t2, feature2, unary)
    ...
    def build_tree(self, dataset, visual_tree, depth=0):
        ...
        if num_samples >= self.min_samples_split and depth <= self.max_depth:
            ...
            if best_split["info_gain"] > 0:
                if best_split["unary"]:
                    visual_child = TreeVisualization(f'{best_split["feature"][0]} ≤ {best_split["threshold"][0]}')
...
if __name__ == "__main__":
    ...
    def heart():
        ...
        tree = MultivariateDecisionTree(data, "death", max_depth=3)
        tree.make()
        tree.print_tree()
    ...
    def pumpkin():
        ...
        tree = MultivariateDecisionTree(data, "class", max_depth=float("inf"))
        tree.make()
        tree.print_tree()
